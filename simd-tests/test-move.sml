...
local
  ...
  fun print_test_scalar (actual:S.simdReal,expected:real,
    ...
    val _ = TextIO.print 
              (concat(["Results for testing ",test,":\n",
                       "Expected result: ",Real32.toString(expected),"\n",
  ...
  val c1:real array = Array.fromList
                        ([423.84863, 867.98645, 362.71906, 357.47028, 797.477,
                          515.48016, 481.2944, 486.26483, 949.5173, 784.454,
                          844.17535, 310.95575, 347.7303, 342.37338, 603.8262,
                          412.0921])
  val C_c1:(int -> simdConst) = fn x => toConst(c1,x)
  val c2:real array = Array.fromList 
                        ([594.5598, 63.665627, 633.1009, 553.8459, 744.85754,
                          92.342735, 298.2279, 194.26346, 988.1369, 646.9155,
                          643.80646, 389.65714, 250.3655, 634.1989, 527.49994,
                          900.44403])
  val C_c2:(int -> simdConst) = fn x => toConst(c2,x)
  val d1:real array = Array.fromList
                        ([0.9751378, 0.2926259, 0.5879499, 0.41872954,
                          0.84174955, 0.5093733, 0.615062, 0.5520501,
                          0.4115485, 0.35940528,0.0056368113, 0.31019592,
                          0.4214077, 0.32522345, 0.2879219, 0.23799527])
  val C_d1:(int -> simdConst) = fn x => toConst(d1,x)
  val d2:real array = Array.fromList
                        ([0.98117805, 0.06889212, 0.32721102, 0.8774886,
                          0.25179327, 0.76311684, 0.42041814, 0.5310335,
                          0.10243285, 0.59039974, 0.072882414, 0.3737452,
                          0.8174622, 0.9795288, 0.9159782, 0.8911723])
  ...
  fun gen_expected_sqrt (arr,i) = 
      toConst(Array.fromList
                (List.tabulate
                   (4, fn x => Real32.Math.sqrt(Array.sub(arr,x+i)))),0)
in
