...
void features(){
  ...
  aes,osxsave,avx,fma};*/
  //flags = mmx,fxsave,sse,sse2,sse3,ssse3,sse4.1,sse4.2,aes,osxsave,avx,fma
  volatile unsigned char flags[12]={0x80,0x1,0x2,0x4,0x1,0x2,0x8,0x16,0x2,0x8,0x10,0x10};
  flags[0]&=edx.bytes[2];
  flags[1]&=edx.bytes[3];
  flags[2]&=edx.bytes[3];
  flags[3]&=edx.bytes[3];
  ...
  volatile unsigned char avx2 = 0x20 & ebx.bytes[0];
  printf("mmx: %hhx, fxsave: %hhx, sse: %hhx, sse2:%hhx\n"
         ...
         flags[8],flags[9],flags[10],flags[11],avx2);
}
